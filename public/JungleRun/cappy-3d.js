(function () {
  ...
  const STAGES = Object.freeze([
    {
      name: 'EMERALD CANOPY', deck: 'the warm-up run', threshold: 0,
    ...
    {
      name: 'RIVER GORGE', deck: 'water cuts the path', threshold: 350,
      sky: 0x50a5c5, path: 0xa75b45, pathLight: 0xd78c58, leaf: 0x154b4c,
      leafLight: 0x70bb72, accent: 0x57d4d0, fogNear: 36, fogFar: 124,
      speedBonus: 1.4, safeGap: 1, turnGap: 10, threatMin: 21, threatRange: 8, difficulty: 2,
    },
    {
      name: 'SUNSET RUINS', deck: 'old stones wake up', threshold: 750,
      sky: 0xd47761, path: 0x95503d, pathLight: 0xd68242, leaf: 0x274a3a,
      leafLight: 0xc2c45b, accent: 0xf1ad38, fogNear: 32, fogFar: 112,
      speedBonus: 2.8, safeGap: 0, turnGap: 12, threatMin: 18, threatRange: 7, difficulty: 3,
    },
    {
      name: 'MOONLIT PULSE', deck: 'the jungle goes electric', threshold: 1200,
      sky: 0x554f86, path: 0x7f4e57, pathLight: 0xb66e6d, leaf: 0x173a38,
      leafLight: 0x7fb85e, accent: 0x59d7c6, fogNear: 27, fogFar: 102,
  ...
  function setPlayerPose(name, force) {
    ...
    const texture = player.poseTextures[name];
  ...
  function makeSegment() {
    return { root: new THREE.Group(), obstacles: [], pickups: [], turn: null };
  }
  ...
  function buildSegment(segment, serial, zPosition, safe) {
    ...
    const fruitSegment = !safe && serial >= 6 && serial % 8 === 4;
    ...
    } else if (fruitSegment) {
      const fruitLane = Math.floor(seeded(serial * 31) * 3);
      addCoinTrail(segment, fruitLane, -8, 7, 0.82);
      addPickup(segment, 'FRUIT', fruitLane, 3.1, 1.05);
    } else if (shouldTurn) {
  ...
  function addJungleSides(root, serial) {
    ...
    for (let z = -9; z <= 9; z += 4.6) {
      [-1, 1].forEach(function (side) {
        ...
        if (random() < 0.68 + stage.difficulty * 0.03) root.add(createTree(x, z + random() * 1.4, random()));
        if (random() < 0.28 + (stage.difficulty === 2 ? 0.26 : 0.08)) root.add(createLeafFan(side * (3.9 + random()), z + random() * 2, random()));
        if (random() < 0.1 + stage.difficulty * 0.08) root.add(createRuin(side * (5.2 + random()), z + 1, random()));
        if (random() < 0.44) root.add(createComicFlower(side * (3.8 + random() * 1.6), z + random() * 2.2, random()));
        if (random() < 0.1 + stage.difficulty * 0.07) root.add(createInkFrond(side * (6.4 + random()), z - 1, side));
      });
    }
  }
  ...
  function addTurnGate(segment, direction) {
    ...
    const branch = inkedMesh(new THREE.BoxGeometry(9.5, 0.16, 7.05), materials.pathLight);
    branch.position.set(direction * 4.7, -0.09, 0);
    gate.add(branch);

    const board = inkedMesh(new THREE.BoxGeometry(4.5, 2.2, 0.28), materials.sun);
    board.position.set(0, 2.45, -4.5);
    gate.add(board);
    const arrowTexture = createArrowTexture(direction);
    const arrow = new THREE.Mesh(
      new THREE.PlaneGeometry(3.5, 1.45),
      new THREE.MeshBasicMaterial({ map: arrowTexture, transparent: true, side: THREE.DoubleSide })
    );
  ...
  function createArrowTexture(direction) {
    ...
    context.fillText(direction < 0 ? '←' : '→', 256, 95);
  ...
  function updateRunning(dt, timestamp) {
    ...
    if (state.boosting) state.boostBonus += state.speed * dt;
    const stage = STAGES[state.stageIndex];
  ...
  function collectPickup(pickup, position) {
    ...
    if (pickup.type === 'COIN') {
      state.coins += 1;
      state.pickupScore += 12 * (state.boosting ? 2 : 1);
      state.streak += 1;
      ...
      state.pickupScore += 45 * (state.boosting ? 2 : 1);
      ...
      state.pickupScore += 60 * (state.boosting ? 2 : 1);
  ...
  function beginTurn(turn) {
    ...
    state.streak += 4;
    state.combo = Math.min(5, 1 + Math.floor(state.streak / 8));
    state.turnAnimation = { direction: turn.direction, time: 0, duration: CONFIG.turnDuration };
  ...
  function updateTurnAnimation(dt) {
    ...
    const followArc = Math.sin(progress * Math.PI);

    world.rotation.y = animation.direction * followArc * Math.PI / 3;
    camera.position.set(
      -animation.direction * followArc * 1.4,
      5 + followArc * 0.32,
      8.6 - followArc * 0.9
    );
    camera.rotation.z = -animation.direction * followArc * 0.065;
    camera.lookAt(animation.direction * followArc * 0.55, 0.95, -10 + followArc * 1.2);
    player.rig.position.x = player.x + animation.direction * followArc * 0.52;
    player.rig.rotation.z += -animation.direction * followArc * 0.14;

    if (progress >= 1) {
      // Re-anchor after dropping any already-pooled future turn gates.
  ...
  function showTurnPrompt(direction) {
    state.stageBannerTimer = 0;
    dom.stageBanner.classList.add('hidden');
    dom.turnArrow.textContent = direction < 0 ? '↰' : '↱';
    dom.turnLabel.textContent = direction < 0 ? 'LEFT TURN' : 'RIGHT TURN';
  ...
  function updateStageProgress() {
    let nextIndex = state.stageIndex;
    for (let i = STAGES.length - 1; i >= 0; i -= 1) {
      if (state.distance >= STAGES[i].threshold) {
        nextIndex = i;
        break;
      }
    }
  ...
  function applyStage(index, announce) {
    ...
    if (announce && !state.activeTurn && !state.turnAnimation) {
      const stage = STAGES[state.stageIndex];
      dom.stageIssue.textContent = 'ISSUE ' + String(state.stageIndex + 1).padStart(2, '0');
  ...
  function applyBiome(heading) {
    ...
    if (backdrop) {
      ...
      backdrop.sun.material.color.setHex(state.stageIndex === 3 ? 0xe96372 : (state.stageIndex === 1 ? 0xdaf2d8 : 0xf5bd31));
      backdrop.rays.forEach(function (ray, index) {
        ray.material.color.setHex(index % 2 ? stage.accent : (state.stageIndex === 3 ? 0xe96372 : 0xf5bd31));
        ray.material.opacity = state.stageIndex === 3 ? 0.38 : 0.28;
      });
  ...
  function updateCrash(dt) {
    ...
    const fallingIntoPit = state.crashReason === 'pit';
  ...
  function crash(reason, collision) {
    ...
    if (reason !== 'missed turn' && state.superchargeTimer > 0) {
    ...
    if (reason !== 'missed turn' && state.shield > 0) {
      state.shield = 0;
      state.streak = 0;
      state.combo = 1;
      impact();
    ...
    dom.gameOverKicker.textContent = reason === 'missed turn' ? 'Wrong way, jungle explorer' : 'The jungle wins this round';
    dom.shell.classList.remove('turning', 'supercharged');
  ...
  function startRun() {
    ...
    dom.shell.classList.remove('turning', 'crashing', 'boosting', 'supercharged', 'impact');
    updateHud();
  ...
  function finishRun() {
    ...
    dom.shell.classList.remove('running', 'high-speed', 'turning', 'boosting', 'supercharged', 'crashing');
    stopBoost();
    clearThreat();
  ...
  function setupControls() {
    document.addEventListener('keydown', function (event) {
      ...
      if (localTest && key === 'm' && state.mode === GAME.RUNNING && segments.length) {
    ...
    window.CappyRunner = {
      ...
      actions: {
        ...
        forceTurn: function (direction) {
          ...
          showTurnPrompt(state.activeTurn.direction);
        },
